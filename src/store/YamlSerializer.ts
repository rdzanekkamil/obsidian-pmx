...
export function hydrateTasks(raw: unknown[]): Task[] {
  if (!Array.isArray(raw)) return [];
  return raw.map(r => hydrateTask(r as Record<string, unknown>));
}

function hydrateTask(r: Record<string, unknown>): Task {
  return makeTask({
    id: r.id as string,
    ...
    subtasks: Array.isArray(r.subtasks) ? hydrateTasks(r.subtasks as unknown[]) : [],
