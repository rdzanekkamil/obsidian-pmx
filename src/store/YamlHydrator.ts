import type { Project, Task, SavedView, CustomFieldDef, ViewMode } from '../types'
...
/**
 * Hydrate a task from frontmatter and (optionally) its body. Pass `body: null`
 * when the body hasn't been read yet — the task's description starts empty and
 * `descriptionLoaded` is false, signaling that callers must load the body
 * before saving or showing the description.
 */
export function hydrateTaskFromFile(
  ...
  const task = mapRawToTask(frontmatter, {
    description: body !== null ? stripAutoGeneratedContent(body) : '',
    descriptionLoaded: body !== null,
    filePath
  })
...
/**
 * Build a Project from frontmatter and (optionally) body. Pass `body: null`
 * when the body hasn't been read yet — description starts empty and
 * `descriptionLoaded` is false.
 */
export function hydrateProjectFromFrontmatter(
  ...
  return {
    ...
    description: (frontmatter.description as string) ?? (body !== null ? body.trim() : ''),
